"""Bounded configurable-project rehearsal across fresh processes on local Windows/NTFS."""
import contextlib
import hashlib
import io
import json
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        ...
        try:
            if mode == "build":
                ...
                assert session.execute_intent(obj, PythonProjectIntentV2("Example", ["lines", "words"])).completed
            else:
...
def run():
    ...
    with tempfile.TemporaryDirectory(prefix=".wo138-probe-", dir=ROOT) as temporary:
        ...
        assert evidence["metrics"] == ["words", "lines"]
        assert evidence["template_id"] == "text_stats_project_v2"
        assert evidence["intent_version"] == evidence["template_version"] == 2
        assert evidence["verifier"] == "trusted_project_cases_v2"
        intent = saved["plans.json"][0]["tasks"][0]["intent"]
        assert intent["metrics"] == ["words", "lines"] and intent["version"] == 2
        before = fingerprint(root)
        app = invoke(["-E", "-s", "-B", str(project / "app.py"), "--text", "one two\nthree"], 0, project)
        assert json.loads(app) == {"words": 3, "lines": 2}
        ...
        for mode in ("default", "old_project", "project_root_only"):
            before = fingerprint(root)
    ...
    print(json.dumps({"fresh_top_level_processes": len(codes), "exit_codes": codes,
                      "four_file_build_use_discovery": "passed", "normalized_saved_metrics": ["words", "lines"], "disabled_recovery_modes": 3,
